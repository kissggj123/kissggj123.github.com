...
if ($(window).width() < 770) {
    var get_answer = confirm("Yumiko Search 是一个聚合性搜索引擎，可以同时对 2 个搜索引擎展开搜索，页面一分为二，充分利用屏幕资源。\n\n然而。。。\n\n你的屏幕实在是太小了，请在电脑或平板上使用 Yumiko搜索，相信会带给你一份相当棒的体验！\n\nhttp://h2y.github.io/allso/\n\n----------\n点击[是]，将跳转到必应手机版：");
    if (get_answer) {
        var hash = getHash();
        if(hash)
...
function so() {
    if ($.trim(soinput_obj[0].value) !== '') {
        ...
        window.document.title = sowhat_str + ' - Yumiko搜索';
    }
    else {
        location.hash = '';
        window.document.title = 'Yumiko搜索';
    }
